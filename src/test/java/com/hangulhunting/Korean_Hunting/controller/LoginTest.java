...
public class LoginTest {
	...
	void loginSuccess() {
		...
		BasicController basicController = new BasicController(userService);

		User user = new User("test1", "1234", "test1@gmail.com", "iit");

		when(userService.loginProcess(user)).thenReturn(new UserResDto("로그인 성공 : " + user.getUserId()));
		...
		Assertions.assertThat("로그인 성공 : " + user.getUserId()).isEqualTo(responseEntity.getBody().getMessage());
	}
	...
	void loginFailed() {
		...
		when(userService.loginProcess(user)).thenThrow(new RuntimeException("로그인 실패"));

		ResponseEntity<UserResDto> responseEntity = basicController.login(user);

		Assertions.assertThat(HttpStatus.UNAUTHORIZED).isEqualTo(responseEntity.getStatusCode());
		Assertions.assertThat("로그인 실패 : " + user.getUserId()).isEqualTo(responseEntity.getBody().getMessage());
	}
